...
- 2026-08-10: added a public `/evidence` route that separates demo-runtime configuration from sanitized live verification, links each claim to its proof, and keeps incomplete Skill and Discord claims visibly withheld.
- 2026-08-11: upgraded `/evidence` with a responsive, source-linked seven-handoff proof spine, an explicit isolated-moderation counterfactual, and direct product-film/demo paths; Playwright now asserts the surface on desktop and mobile. The live-auth harness moved to an overridable dedicated port so it cannot contend with the private runtime.
- 2026-08-11: added a versioned, incident-scoped Decision Receipt with a print/PDF surface, downloadable canonical JSON, SHA-256 payload integrity, explicit runtime disclosures, creator-authenticated live access, and regression coverage for scope, secret minimization, desktop/mobile layout, and live fail-closed behavior.
- 2026-08-07: persisted autonomy was hardened with typed evidence-bearing outcomes, repository-derived community ownership, live rejection of seeded demo evidence, community-scoped pulses, and regression coverage for live retry/failure/completion paths.
- 2026-08-08: live autonomous follow-up gained a grounded Discord observation path through the persistent Mind, including persisted source-channel ownership, strict follow-up schemas, message-reference validation, a 0.75 resolution threshold, single-worker claim ownership, and fail-closed tests.
- 2026-08-08: live dashboard access gained a single-creator signed session boundary, exact-origin mutation checks, bounded login throttling, accessible sign-in/sign-out UX, security headers, and an independent live-mode browser test.
